...
spam_word_sum = x_train[train_df["Prediction"] == 1].sum().sum() + (alpha * len(x_train.columns))
theta_spam = (x_train[train_df["Prediction"] == 1].sum().values + alpha)/spam_word_sum

normal_word_sum = x_train[train_df["Prediction"] == 0].sum().sum() + (alpha * len(x_train.columns)) 
theta_normal = (x_train[train_df["Prediction"] == 0].sum().values + alpha) / normal_word_sum
...
def classify(document_features):
    ...
    log_spam = np.log(theta_spam)
    log_spam[ log_spam == -math.inf] = -1e12
    probs[1] = np.log(1 - pi_normal) + np.sum(feature_arr * log_spam)
    
    
    log_normal = np.log(theta_normal)
    log_normal[ log_normal == -math.inf] = -1e12
    probs[0] = np.log(pi_normal) + np.sum(feature_arr * log_normal)
...
def performance_metrics(test_x, test_y):
    prediction = x_test.apply(classify, axis=1)
...
output =f"""
Multinomial Naive Bayes Mode with Dirichlet Prior:
alpha = {alpha}
